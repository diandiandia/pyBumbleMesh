...
static const struct cfg_cmd cmds[] = {
	...
	{ OP_REM_PROV_SCAN_START, OP_REM_PROV_SCAN_STATUS, "RemoteProvScanStart" },
	{ OP_REM_PROV_SCAN_STATUS, NO_RESPONSE, "RemoteProvScanStatus" },
	{ OP_REM_PROV_SCAN_REPORT, NO_RESPONSE, "RemoteProvScanReport" },
	{ OP_REM_PROV_LINK_OPEN, OP_REM_PROV_LINK_STATUS, "RemoteProvLinkOpen" },
	{ OP_REM_PROV_LINK_STATUS, NO_RESPONSE, "RemoteProvLinkStatus" },
	{ OP_REM_PROV_PDU_REPORT, NO_RESPONSE, "RemoteProvPduReport" }
};
							...
							uint16_t len)
{
	...
	case OP_REM_PROV_SCAN_STATUS:
		bt_shell_printf("Remote Scan Status from %4.4x: %s\n",
				src, (data[0] == 0) ? "Success" : "Failed");
		return true;

	case OP_REM_PROV_SCAN_REPORT:
		if (len < 17)
			return true;
		bt_shell_printf("Remote Scan Report from %4.4x:\n", src);
		bt_shell_printf("\tRSSI: %d\n", (int8_t)data[0]);
		{
			char *uuid_str = l_util_hexstring_upper(data + 1, 16);
	...
	case OP_REM_PROV_LINK_STATUS:
		if (len < 1)
			return true;
		bt_shell_printf("Remote Link Status from %4.4x: %s (Status: %d)\n",
				src, (data[0] == 0) ? "Success" : "Error", data[0]);
		return true;

	case OP_REM_PROV_SCAN_CAP_STATUS:
		if (len < 2)
			return true;
		bt_shell_printf("Remote Scan Capabilities from %4.4x:\n", src);
		bt_shell_printf("\tMax Scans: %d\n", data[0]);
		bt_shell_printf("\tSupport Active Scan: %s\n", data[1] ? "Yes" : "No");
		return true;

	case OP_REM_PROV_PDU_REPORT:
		bt_shell_printf("Remote PDU Report Received from %4.4x (len %u)\n",
				src, len);
		return true;
	}
...
static void cmd_rpr_scan(int argc, char *argv[])
{
	uint16_t n;
	uint8_t msg[32];
	uint32_t parm_cnt;
	...
	n = mesh_opcode_set(OP_REM_PROV_SCAN_START, msg);

	parm_cnt = read_input_parameters(argc, argv);
	/* Scan Limit (1), Timeout (1) */
	msg[n++] = (parm_cnt >= 1) ? parms[0] : 0x00;
	msg[n++] = (parm_cnt >= 2) ? parms[1] : 0x0A;

	if (!config_send(msg, n, OP_REM_PROV_SCAN_START))
		return bt_shell_noninteractive_quit(EXIT_FAILURE);

	return bt_shell_noninteractive_quit(EXIT_SUCCESS);
}

static void cmd_rpr_scan_stop(int argc, char *argv[])
{
	uint16_t n;
	uint8_t msg[32];
	...
	n = mesh_opcode_set(OP_REM_PROV_SCAN_STOP, msg);

	if (!config_send(msg, n, OP_REM_PROV_SCAN_STOP))
		return bt_shell_noninteractive_quit(EXIT_FAILURE);

	return bt_shell_noninteractive_quit(EXIT_SUCCESS);
}

static void cmd_rpr_link_open(int argc, char *argv[])
{
	uint16_t n;
	uint8_t msg[32];
	size_t sz;
	...
	if (argc < 2) {
		bt_shell_printf("UUID required\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}

	uuid = l_util_from_hexstring(argv[1], &sz);
	if (!uuid || sz != 16) {
		l_free(uuid);
		bt_shell_printf("Invalid UUID\n");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}

	n = mesh_opcode_set(OP_REM_PROV_LINK_OPEN, msg);
	memcpy(msg + n, uuid, 16);
	n += 16;
	l_free(uuid);

	if (!config_send(msg, n, OP_REM_PROV_LINK_OPEN))
		return bt_shell_noninteractive_quit(EXIT_FAILURE);

	return bt_shell_noninteractive_quit(EXIT_SUCCESS);
}

static void cmd_rpr_link_close(int argc, char *argv[])
{
	uint16_t n;
	uint8_t msg[32];
	...
	n = mesh_opcode_set(OP_REM_PROV_LINK_CLOSE, msg);
	/* Reason (1) */
	msg[n++] = (argc > 1) ? (uint8_t)strtol(argv[1], NULL, 16) : 0x00;

	if (!config_send(msg, n, OP_REM_PROV_LINK_CLOSE))
		return bt_shell_noninteractive_quit(EXIT_FAILURE);

	return bt_shell_noninteractive_quit(EXIT_SUCCESS);
}

static void cmd_rpr_cap_get(int argc, char *argv[])
{
	uint16_t n;
	uint8_t msg[32];
	...
	n = mesh_opcode_set(OP_REM_PROV_SCAN_CAP_GET, msg);

	if (!config_send(msg, n, OP_REM_PROV_SCAN_CAP_GET))
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
...
static const struct bt_shell_menu cfg_menu = {
	...
	{"rpr-get-caps", NULL, cmd_rpr_cap_get,
				"Remote Provisioning Get Capabilities"},
	{"rpr-scan", "[limit] [timeout]", cmd_rpr_scan,
				"Remote Provisioning Scan Start"},
	{"rpr-scan-stop", NULL, cmd_rpr_scan_stop,
				"Remote Provisioning Scan Stop"},
	{"rpr-link-open", "<uuid>", cmd_rpr_link_open,
				"Remote Provisioning Link Open"},
	{"rpr-link-close", "[reason]", cmd_rpr_link_close,
